 ...
 * a simple mechanism to inject an EM into a thread.<br>
 * <br>
 * NOTE: You need to initialise this Provider during application startup by
 * calling EntityManagerProvider.setEntityManagerFactory().<br>
 * <br>
 * You shouldn't use this provider directly but rather use it via one of:<br>
 * <br>
 * EntityManagerCallable EntityManagerRunnable EntityManagerThread<br>
 * <br>
 * EntityManagerInjectorFilter AtmosphereFilter<br>
 * <br>
 * Each of these class correctly sets up the EM, starts a Transaction and
 * finally clears the threads EM once the thread is complete.<br>
 * <br>
 * If you want to use EntityManagerProvider directly (DON'T):<br>
 * <br>
 * Then for each thread you need to inject a thread local EntityManager by
 * calling setCurrentEntityManager<br>
 * and then clearing it when the thread shuts down by calling
 * setCurrentEntityManager with a null.<br>
 * You will also need wrap the entity in a Transaction block before calling
 * setCurrentEntityManager.<br>
 * <br>
 * e.g.
 * 
 * <pre>
 ...
 * try {
 *     // Create and set the entity manager<br>
...
public enum EntityManagerProvider {
    ...
    public static void setCurrentEntityManager(EntityManager em) {
        ...
        /**
         * Before you try to clear or replace the current entity manager you should do
         * something similar to:
         *
         * @formatter:off try { transaction.commit(); transaction.close(); em.close(); }
         *                finally { EntityManagerProvider.setCurrentEntityManager(null);
         *                }
         * @formatter:on
         *
         */
     ...
     * out EntityManagers to worker threads. Dont forget to close the entitymanager
    ...
    /**
     * provides the same functionality as setThreadLocalEntityManager(EntityWorker
     * w), without the need for an anonymous inner class<br>
     * example usage:<br>
     * <br>
    ...
    /**
     * Allows you to pass a Runnable to wrap in an entity manager. A new Runnable is
     * returned which should then be called to run your runnable. i.e. don't run you
     * own runnable directly rather use the returned Runnable.
     *
     * @param runnable - the runnable to run as contains an entity manager.
     * @return
     */
     ...
     * entity manager (as they are injected by the servlet request filters mentioned
     * above. <br>
     * <br>
    ...
    public static EntityManager createEntityManager() {
        ...
        final EntityManager entityManager = INSTANCE.emf.createEntityManager();
     ...
     * cleared the Runnable must NOT try any database operations as they will fail.
     *
    ...
    private static void runTransientPostActions() {
        ...
        final List<Runnable> actions = transientPostTransactionActions.get();
    ...
    /**
     * this is useful when you need to run code in a separate transaction and thread
     ...
     * transaction. Registered actions are global and will be run across all threads
     * every time an entity manager is set via void
     ...
     * transaction.
     *
     * Registered actions are global and will be run across all threads every time
     * an entity manager is set via a call to
     *
    ...
    /**
     * Evicts the entity of the specified class with the specified key in the JPA
     * cache
     */
