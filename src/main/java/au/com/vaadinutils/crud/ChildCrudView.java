...
public abstract class ChildCrudView<P extends CrudEntity, E extends ChildCrudEntity> extends BaseCrudView<E>
    ...
    /**
     * Load the page with a bogus parent filter to prevent possible large queries
     * from being executed before a parent row is selected
     */
    ...
    /**
     * override this method and return ture if this crud is for a highly contended
     * table.
     * 
     * @return
     */
     ...
     *
     * we have to hook the ItemSetChangeListener to be able to get the database id
     * of a new record.
     */
    ...
    /**
     * if a row is deleted in the childCrud, then it is dirty for the purposes of
     * the parent crud
     */
    @Override
    public void delete() {
    ...
    /**
     * Need to over-ride as the rules display of a child's new button a different to
     * those of the parent.
     */
    @Override
     ...
     *
     *
     * When adding fields to a layout that need to be committed, but are not part of
     * the field group, override this method to let the crud know that a commit
     * should be performed.
     *
     * This is required because committing on a child crud is not explicit -no save
     * button is available, the need to save is determined based on if the fields
     * are dirty.
     *
     * @return
     */
    ...
    /**
     * this method is called when the parent crud changes row, so we set filters so
     * that this child only displays the associated rows
     */
    @Override
    ...
    /**
     * the id (Long) of the parent will need to be translated into an entity for the
     * filtering. an implementing class must implement this method and return an
     * instance of the Parent class (P) with it's id set (parentId2)
     *
     * @param parentId2
     * @return
